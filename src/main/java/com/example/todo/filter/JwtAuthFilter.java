...
//    필터가 해야할 작업을 기술
    ...
    protected void doFilterInternal(HttpServletRequest request,
        ...
        try {
            ...
            if(token != null) {
                ...
                //인증 완료 처리(스프링 시큐리티에게 인증정보 전달해서 전역적으로 앱에서 인증정보를 활용하도록 설정)
                AbstractAuthenticationToken auth
                         = new UsernamePasswordAuthenticationToken(
                                 userInfo, // 컨트롤러에서 활용할 유저정보
                                 null, //인증된 사용자의 비밀번호. 보통은 null값을 설정함
                                authorityList //인가 정보(권한정보). 선택사항
                );
    ...
    private static String parseBearerToken(HttpServletRequest request) {
        //클라이언트에서 보내는 http header에서 요청 헤더에서 토큰 가져오기
